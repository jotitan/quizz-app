...
func (fgs filerQuizzStorage) ReadMusic(quizz model.Quizz, questionId string, writer io.Writer) error {
	...
	path := filepath.Join(fgs.generateAssetFolder(quizz), quizz.Questions[pos].MusicPath)
...
func (fgs filerQuizzStorage) StoreMusic(quizz model.Quizz, musicFile string, from, to int) (string, string, error) {
	path, name, err := fgs.generateMusicPath(quizz)
	if err != nil {
		return "", "", err
	}
	return path, name, fgs.musicCutter.Cut(musicFile, path, from, to)
}

func (fgs filerQuizzStorage) generateAssetFolder(quizz model.Quizz) string {
	return filepath.Join(fgs.filer, "assets", quizz.Id)
}

func (fgs filerQuizzStorage) generateMusicPath(quizz model.Quizz) (string, string, error) {
	folder := fgs.generateAssetFolder(quizz)
	if err := os.MkdirAll(folder, os.ModePerm); err != nil {
		return "", "", err
	}
	name := fmt.Sprintf("%s.mp3", generateUniqueIdAsString())
	return filepath.Join(folder, name), name, nil
}
